"""WIO Bank statement parser."""
from pathlib import Path
import pandas as pd
from .base_parser import BankParser


class WIOParser(BankParser):
    """Parser for WIO Bank statements."""
    
    BANK_NAME = "WIO"
    SUPPORTED_FORMATS = ['.xlsx', '.csv', '.xls', '.pdf']
    
    def can_parse(self, file_path: Path) -> tuple[bool, float]:
        """
        Detect if this is a WIO statement.
        
        TODO: Implement detection logic once we have sample files.
        """
        if file_path.suffix.lower() not in self.SUPPORTED_FORMATS:
            return (False, 0.0)
        
        # Skip PDFs for now (would need PDF parsing library)
        if file_path.suffix.lower() == '.pdf':
            return (False, 0.0)
        
        try:
            # Read first few rows
            if file_path.suffix.lower() == '.csv':
                df = pd.read_csv(file_path, nrows=10)
            else:
                df = pd.read_excel(file_path, nrows=10)
            
            # Look for WIO-specific indicators
            text_content = ' '.join(df.astype(str).values.flatten()).upper()
            
            if 'WIO' in text_content or 'WIO BANK' in text_content:
                return (True, 0.8)
            
            return (False, 0.0)
            
        except Exception:
            return (False, 0.0)
    
    def parse(self, file_path: Path) -> pd.DataFrame:
        """
        Parse WIO statement.
        
        TODO: Implement parsing logic once we have sample files.
        For now, raise an error to trigger unparsed file storage.
        """
        raise ValueError(
            "WIO parser not yet implemented. "
            "File has been saved for admin review. "
            "A parser will be created based on this file format."
        )

